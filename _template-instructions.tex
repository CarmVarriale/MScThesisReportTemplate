...
Please check the corresponding source code in the \texttt{.tex} files to see how they have been implemented, and reuse it.
...
For subscripts and superscripts, use the macros defined in the \texttt{.preamble/symbols-macros.tex} file, which are designed to produce a consistent style throughout the document.
For example:
\begin{itemize}
    \item If a subscript symbol is a word, the initial letter of a word, or an acronym, use the ``roman'' font type of the macros or the \texttt{\textbackslash{}mathrm\{\}} command to have it in upright font: $\gsubrm{V}{eas}, \gsubrm{V}{eq}, \gsub{m}{mto}, \gsubrm{W}{ini}, \gsubrm{W}{fin}, \gsubrm{C}{D}, \gsubrm{C}{L}, \gsubrm{C}{M}, \gsubrm{T}{max}, \gsubrm{T}{min}$.
    \item If a subscript symbol is a symbol itself, use the regular ``math'' font: $\gsub{V}{alpha}, \gsub{C}{L}, \gsubsub{C}{D}{M}$.
\end{itemize}
%
By using the command macros correctly, the List of Symbols and List of Acronyms will be automatically consistent and well formatted.
...
\section{Examples}
Some reports that have been written using this template and that you can use for inspiration and reference for your own work are listed in \Cref{tab:example-reports}.

\begin{table}[bhtp]
    \centering
    \caption{Example reports for inspiration and reference.}
    \label{tab:example-reports}
    \begin{tabular}{lll}
        \toprule
        \textbf{Author} & \textbf{Title (with link)} & \textbf{Year}\\
        \midrule
        Rik Lambrichs 
        & \href{https://resolver.tudelft.nl/uuid:8b3714f7-8ab0-4cc2-b0ea-5b9d02bd325f}{Non-linear Dynamics of the Flared Folding Wingtip Concept} 
        & 2026 
        \\
        \bottomrule
    \end{tabular}
